package com.ai.baas.op.sandbox.util;
...
import com.ai.baas.op.base.util.DateUtil;
...
public class ReflectUtil {
	...
	public static Map<Object, Object> reflectClass(String indexId) {  
		...
		// 获得APIClassDoc
		APIClassDoc doc = JSONObject.parseObject(DubboApiSearchUtil.getAPIClassDetail(indexId), APIClassDoc.class);
		if(null!=doc){
			...
			for (APIClassFieldDoc fdoc : flist) {
				if (fdoc.isCanUnfold()) {
					...
					//APIClassDoc doc = JSONObject.parseObject(id, APIClassDoc.class);
					...
					if(fdoc.getClassName().equals(HashMap.class.getName())){
						...
						map.put(fdoc.getClassName().substring(fdoc.getClassName().lastIndexOf(".")+1), reflectClass(id));
					}
	...
	public static void main(String[] args) {
		...
		doc.setBelong("123");
		doc.setCanUnfold(false);
		doc.setClassName("java.lang.String");
		doc.setCommentText("注释");
		doc.setId(123456);
		doc.setName("id");
		doc.setVersion("1.0");
		reflectClass(doc);
		...
		//System.out.println(JSON.toJSON(reflectClass1("1974627168")));
	}
}
